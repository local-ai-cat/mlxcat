...
|---|---|---|---|
| `MLXCAT_TOOL_GRAMMAR` + per-request `tool_grammar` | off (`QwenXMLToolGrammarPolicy.defaultEnabled`) | Qwen-XML tool-call bodies constrained to well-formed arguments (the unescaped-quote failure becomes unreachable; 5/5 live smoke). **Mask p99 0.29ms body / 4.1ms trigger (was 110), −0.68% flag-off throughput (single sample); speculation pauses inside tool bodies. Validity-vs-LM-Studio unpriced — default stays off until the A/B lands.** Env `0/off/false` is the operator kill switch and beats a requesting client; `tool_grammar: true/false` in the request body overrides the default on chat/messages/responses. Exit: delete `TrackB/Grammar/QwenXMLToolGrammar.swift` + the branch at `Sampling.swift` `toolGrammarMatcher` and the arm site in `NativeModelEngine.toolGrammar(from:)`. | `MLXCatNative/NativeModelEngine.swift:347` |
