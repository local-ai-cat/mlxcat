...
|---|---|---|---|
| `MLXCAT_TOOL_GRAMMAR` + per-request `tool_grammar` | **on** (`QwenXMLToolGrammarPolicy.defaultEnabled`; `0` disables) | Qwen-XML tool-call bodies constrained to well-formed arguments (the unescaped-quote failure becomes unreachable; 5/5 live smoke). **Mask p99 0.29ms body / 4.1ms trigger (was 110). A/B 2026-08-29 (12 quote-heavy prompts, Qwen3-Coder-30B): grammar-on t=0.7 60.6 tok/s vs 60.7 unconstrained — zero measured cost; no validity regression; LM Studio same weights 47 tok/s. Speculation still pauses inside tool bodies.** Env `0/off/false` is the operator kill switch and beats a requesting client; `tool_grammar: true/false` in the request body overrides the default on chat/messages/responses. Exit: delete `TrackB/Grammar/QwenXMLToolGrammar.swift` + the branch at `Sampling.swift` `toolGrammarMatcher` and the arm site in `NativeModelEngine.toolGrammar(from:)`. | `MLXCatNative/NativeModelEngine.swift:347` |
