...
for pin in pins:
    ...
    report["pins"].append({**pin, "pin_date": pin_date, "head": h, "latest_release": rel,
                           "commits_since_pin": len(subjects), "keyword_hits": keyword_hits(subjects)})
...
for repo, why in WATCH:
    ...
    report["watch"].append({"repo": repo, "why": why, "head": h, "latest_release": latest_release(repo),
                            "commits_in_window": len(subjects), "keyword_hits": keyword_hits(subjects)})
...
for w in report["watch"]:
    ...
    print(f"| {w['repo']} | {w['why']} | `{h['sha']}` {h['date']} | {rel.get('tag') or '—'} {rel.get('date','')} | {w['commits_in_window']} | {len(w['keyword_hits'])} |")
